...
static void testCmykToRgbValues(void **state) {
  ...
  double *values = *((double **)state);
  int r, g, b;
  cmykToRGB( &r, &g, &b, values[3], values[4], values[5], values[6]);
  assert_true(
    isClose(r, values[0]) &&
    isClose(g, values[1]) &&
    isClose(b, values[2])
...
static void testRgbtoGrayscaleAverage(void **state) {
	double *values = *((double **)state);
	toGrayScale(values[0], values[1], values[2], AVERAGE);
}
int main(int argc, char **argv) {

  double aquamarine[] =  {127.0, 255.0, 212.0, 0.50, 0.0, 0.17, 0.0};
  double salmon[] = {250, 128, 114, 0.0, 0.49, 0.54, 0.02, 164};
  ...
  const struct CMUnitTest tests[] = {
	...
	cmocka_unit_test_prestate(testCmykToRgbValues, &aquamarine),
    cmocka_unit_test_prestate(testCmykToRgbValues, &salmon)
  };
